...
imdb_id_mapper = df_a['id'].to_dict()
dbpedia_id_mapper = df_b['id'].to_dict()
...
imdb_embeddings = np.array(df_a['v'].tolist()).astype(np.float32)
dbpedia_embeddings = np.array(df_b['v'].tolist()).astype(np.float32)

print("\n--- 4. Building Faiss index for imdb records ---")
d = imdb_embeddings.shape[1]
...
D, I = index.search(dbpedia_embeddings, k)
...
stage1_pairs_data = []
X_stage1_features = []
for dbpedia_idx, imdb_matches in enumerate(I):
    dbpedia_id = dbpedia_id_mapper[dbpedia_idx]
    dbpedia_embedding = dbpedia_embeddings[dbpedia_idx]
    dbpedia_record = df_b.iloc[dbpedia_idx] # Get the full record

    for i, imdb_idx in enumerate(imdb_matches):
       imdb_id = imdb_id_mapper[imdb_idx]
       imdb_embedding = imdb_embeddings[imdb_idx]
       imdb_record = df_a.iloc[imdb_idx] # Get the full record
       if imdb_id in truthD:
            v1 = {"v": imdb_embedding}
            v2 = {"v": dbpedia_embedding}
            stage1_pairs_data.append((imdb_record, dbpedia_record))
            ...
            if dbpedia_id in truthD[imdb_id]:
...
if len(stage2_candidate_indices) > 0:
    ...
    for idx in stage2_candidate_indices:
        ...
        imdb_record, dbpedia_record = stage1_pairs_data[idx]
        
        # Create the SLOW, HYBRID (embedding + jaro) feature vector
        hybrid_features = lib.create_hybrid_feature_vector(imdb_record, dbpedia_record)
        X_stage2_features.append(hybrid_features)
        
        # Get the true label for our final test
        imdb_id = str(imdb_record["id"])
        dbpedia_id = str(dbpedia_record["id"])
        y_true_list.append(1.0 if (imdb_id, dbpedia_id) in gt_lookup else 0.0)
