...
acm_embeddings = np.array(df_a['v'].tolist()).astype(np.float32)
...
print("\n--- 4. Building Faiss index for dblp records ---")
...
D, I = index.search(acm_embeddings, k)
...
stage1_pairs_data = []
X_stage1_features = []
for acm_idx, dblp_matches in enumerate(I):
    acm_id = acm_id_mapper[acm_idx]
    acm_embedding = acm_embeddings[acm_idx]
    acm_record = df_a.iloc[acm_idx] # Get the full record

    for i, dblp_idx in enumerate(dblp_matches):
       dblp_id = dblp_id_mapper[dblp_idx]
       dblp_embedding = dblp_embeddings[dblp_idx]
       dblp_record = df_b.iloc[dblp_idx] # Get the full record
       if acm_id in truthD:
            v1 = {"v": acm_embedding}
            v2 = {"v": dblp_embedding}
...
if len(stage2_candidate_indices) > 0:
    ...
    for idx in stage2_candidate_indices:
        ...
        hybrid_features = lib.create_hybrid_feature_vector(acm_record, dblp_record)
        ...
        y_true_list.append(1.0 if (acm_id, dblp_id) in gt_lookup else 0.0)
