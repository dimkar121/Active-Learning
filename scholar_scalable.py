...
VAL_SET_PROPORTION = 0.1  # Use 50% of the first chunk's candidates
...
print("Building global FAISS index for DBLP (df_b)...")
embeddings_b = np.array(df_b['v'].tolist()).astype('float32')
d = embeddings_b.shape[1]
index_b = faiss.IndexHNSWFlat(d, 32, faiss.METRIC_INNER_PRODUCT)
index_b.hnsw.efConstruction = 60
index_b.hnsw.efSearch = 64
faiss.normalize_L2(embeddings_b) # Normalize for inner product (cosine sim)
index_b.add(embeddings_b)
print("DBLP index built.")
...
for i in range(N_PARTITIONS):
    ...
    faiss.normalize_L2(embeddings_a_partition) # Normalize for search

    # 2. Generate Candidate Pool for THIS partition
    print(f"Generating candidate pool for {len(df_a_partition)} records...")
    D, I = index_b.search(embeddings_a_partition, k=10)
...
D, I = index_a.search(dblp_embeddings, k=5)
