...
NUM_ITERATIONS = 12         # Replaces 3 partitions * 4 iterations
...
ETA_MULTIPLIER = 0.35       # Determines the base capacity eta
...
ID_COL_A = 'idAmazon'
ID_COL_B = 'idGoogle'
COLS_TO_USE = ["name", "description", "manufacturer"] 
...
for i, r in df_gt.iterrows():
        ...
        if idAmazon in truthD.keys():
            truthD[idAmazon].append(idGoogle)
            a += 1
        else:
...
amazon_embeddings = np.array(df_a['v'].tolist()).astype('float32')
df_b_whole = df_b
...
print("Building global FAISS index (Amazon)...")
d = amazon_embeddings.shape[1]
index = faiss.IndexHNSWFlat(d, 32, faiss.METRIC_INNER_PRODUCT)
index.hnsw.efConstruction = 60
index.hnsw.efSearch = 64
faiss.normalize_L2(amazon_embeddings) 
...
ETA = int(ETA_MULTIPLIER * len(google_embeddings) * K_NEIGHBORS)
ETA_B = int(ALPHA * ETA)
...
for i in tqdm(range(len(google_embeddings)), desc="Querying Google Candidates"):
    query_vec = google_embeddings[i:i+1]
    ...
    if len(R_b) == ETA_B and len(R_c) == ETA_C:
        ...
        if insertion_rate < GAMMA and no > ETA:
            print(f"\nDynamic truncation triggered! Stopping at record {i}/{len(google_embeddings)}")
...
# Apply leakage fix: extract Google IDs from only the actively labeled records
...
# Search Google against the Amazon index
...
print("\n" + "="*30)
print("PAPER TABLE 2 DATA (Amazon-Google)")
print("="*30)
