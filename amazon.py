...
google_id_mapper = df_b['id'].to_dict()
...
google_embeddings = np.array(df_b['v'].tolist()).astype(np.float32)
...
D, I = index.search(google_embeddings, k)
...
stage1_pairs_data = []
X_stage1_features = []
for google_idx, amazon_matches in enumerate(I):
    google_id = google_id_mapper[google_idx]
    google_embedding = google_embeddings[google_idx]
    google_record = df_b.iloc[google_idx] # Get the full record

    for i, amazon_idx in enumerate(amazon_matches):
       ...
       if amazon_id in truthD:
            ...
            if google_id in truthD[amazon_id]:
...
if len(stage2_candidate_indices) > 0:
    ...
    for idx in stage2_candidate_indices:
        ...
        hybrid_features = lib.create_hybrid_feature_vector(amazon_record, google_record)
        ...
        y_true_list.append(1.0 if (amazon_id, google_id) in gt_lookup else 0.0)
