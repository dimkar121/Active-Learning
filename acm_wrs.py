...
NUM_ITERATIONS = 9          # Replaces 3 iterations * 3 partitions
...
print("Building global FAISS index (DBLP)...")
d = acm_embeddings.shape[1]
...
ETA = int(ETA_MULTIPLIER * len(acm_embeddings) * K_NEIGHBORS)
ETA_B = int(ALPHA * ETA)
...
for i in tqdm(range(len(acm_embeddings)), desc="Querying ACM Candidates"):
    query_vec = acm_embeddings[i:i+1]
    ...
    if len(R_b) == ETA_B and len(R_c) == ETA_C:
        ...
        if insertion_rate < GAMMA and no > ETA:
            print(f"\nDynamic truncation triggered! Stopping at record {i}/{len(acm_embeddings)}")
...
# --- 5. Final Two-Stage Resolution ---
time_start_res = time.time()
print("\n--- Starting Final Two-Stage Resolution (Held-Out Test Set) ---")

# Fix applied here: Extract ONLY the strictly labeled ACM records to avoid blacklisting the whole DB
...
# Search ACM against the DBLP index
